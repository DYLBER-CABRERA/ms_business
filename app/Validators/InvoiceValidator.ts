...
export default class InvoiceValidator {
  ...
  public schema = schema.create({
    ...
    quota_id: schema.number(
      ...
      expense_id: schema.number([rules.exists({ table: 'expenses', column: 'id' }),
        rules.unsigned(),// Asegura que el ID del contrato no sea negativo
        rules.required()]),// Asegura que el campo no esté vacío
  ...
  public messages: CustomMessages = {
    'date.date': 'La fecha de la factura debe tener el formato yyyy-MM-dd HH:mm:ss.',
    'date.after': 'La fecha de la factura debe ser posterior a la fecha actual.',
